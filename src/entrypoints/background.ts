...
export default defineBackground(() => {
  ...
  async function addShortToTabId(shortId: string, tabId: number) {
    try {
      if (!shortId) {
        console.log("There is no shorts url", tabId);
        deleteTabId(tabId);
        await browser.tabs.sendMessage(tabId, {
          action: "NO_SHORTS_URL",
        });
        return;
      }
      ...
      console.error("Error in addShortToTabId:", error);
    }
  }
  ...
  browser.tabs.onUpdated.addListener(async (tabId, tabInfo, tab) => {
    if (tabInfo.url) {
      if (!tabsShortsMap.has(tabId)) {
        ...
        await addShortToTabId(retrieveShortId(tabInfo.url)!, tabId);
      }
    }
  });
  ...
  browser.runtime.onStartup.addListener(() => {
    browser.storage.local.set({ shortsLimit: 10 });
  });
});
